...
7. Choose the prompt theme: an Oh-My-Posh theme, or a Starship preset
...
├── variables.ps1    # Env vars (DEVKIT_ROOT, DEVKIT_PROMPT_ENGINE, DEVKIT_OMP_THEME, DEVKIT_STARSHIP_CONFIG, DEVKIT_REPO_ROOT)
...
- Top-level subcommands: `help [topic]`, `version`, `doctor`, `find <keyword>`, `update`, `nvim refresh`, `claude refresh [--force]`, `herdr refresh`, `statusline status|install|refresh|size <mode>|remove`, `prompt status|use <engine>|preset <name>|list`, `prereqs check|install`, `backups list|restore`, `fix terminal-icons`.
