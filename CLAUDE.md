...
- Top-level subcommands: `help [topic]`, `version`, `doctor`, `find <keyword>`, `update`, `nvim refresh`, `claude refresh`, `herdr refresh`, `backups list|restore`, `fix terminal-icons`.
...
- `devkit update`, `devkit nvim refresh`, `devkit claude refresh`, and `devkit herdr refresh` dot-source from `$env:DEVKIT_REPO_ROOT` (written into `~/.devkit/variables.ps1` by `Save-VariablesPs1`). If the repo moves, those commands print a clear error; everything else still works. `claude refresh` re-runs the installer's `Copy-DevkitClaude*` + `Install-HerdrHookAndSettings` helpers; `herdr refresh` re-runs `Save-HerdrConfig`.
