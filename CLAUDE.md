...
- Top-level subcommands: `help [topic]`, `version`, `doctor`, `find <keyword>`, `update`, `nvim refresh`, `claude refresh [--force]`, `herdr refresh`, `statusline status|install|refresh|size <mode>|remove`, `prereqs check|install`, `backups list|restore`, `fix terminal-icons`.
...
- The wizard's **step 2** can install the external tools the devkit depends on. Opt-in gated, individually selectable, and it installs **immediately inside the step** rather than in `Invoke-Installation`.
...
- **External installers are invoked from exactly three seams.** `Invoke-WingetInstall`, `Invoke-RemoteScriptInstall` and `Invoke-OhMyPoshFontInstall` in `config-generator.ps1` are the only places a package manager or vendor installer runs, and `Install-Prerequisites` calls them **by name** so `test-prereqs.ps1` can shadow them. Inline `& winget` into the caller and the test suite starts installing software. Specifics a future refactor would otherwise break:
...
- **`$script:WizardTotalSteps` in `wizard.ps1` is the single place the step count lives.** `Show-StepHeader` defaults to it and every call site passes it, so inserting a step is a one-line change rather than nine literal edits.
