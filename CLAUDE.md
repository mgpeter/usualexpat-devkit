...
- Top-level subcommands: `help [topic]`, `version`, `doctor`, `find <keyword>`, `update`, `nvim refresh`, `claude refresh [--force]`, `herdr refresh`, `statusline status|install|refresh|size <mode>|remove`, `backups list|restore`, `fix terminal-icons`.
